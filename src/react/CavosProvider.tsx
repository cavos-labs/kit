...
export function CavosProvider({ config, modal, children }: CavosProviderProps) {
  ...
  const enrollPasskey = useCallback(
    async (passkey: PasskeySigner, params: PasskeyEnrollParams) => {
      if (!cavos) throw new Error('Not logged in');
      // Present on every chain's wallet handle.
  ...
  const approveThisDeviceWithPasskey = useCallback(
    async (
      ...
      if (!cavos) throw new Error('Not logged in');
      if (cavos.chain === 'starknet') {
        // Sponsored through the Cavos paymaster by default; `submit` overrides.
        return cavos.approveThisDeviceWithPasskey({ passkey, ...(submit ? { submit } : {}) });
      }
      // Solana / Stellar submit through their configured relayer.
